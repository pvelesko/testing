            ...
            VectorType& y)
{
  ...
  int groups = 8;
  int items = groups * items_per_group;
  int rows_per_item = (n + items - 1) / items;
#endif
  ...
  Queue.submit([&](cl::sycl::handler &cgh) {
      ...
      auto Acols = A.device_cols.template get_access<cl::sycl::access::mode::read>(cgh);
      auto Acoefs = A.device_coefs.template get_access<cl::sycl::access::mode::read>(cgh);
      auto Arowoffsets = A.device_row_offsets.template get_access<cl::sycl::access::mode::read>(cgh);

      cgh.parallel_for<class matvec>(ndRange, [=](cl::sycl::nd_item<1> item) {
          GlobalOrdinalType starting_row = item.get_global_id(0)*rows_per_item;
          for (GlobalOrdinalType row =  starting_row; row < std::min(n, starting_row + rows_per_item); row++)
            {
              GlobalOrdinalType row_start = Arowoffsets[row];
              GlobalOrdinalType row_end   = Arowoffsets[row+1];
              ScalarType sum = beta*ycoefs[row];
              for(LocalOrdinalType i=row_start; i<row_end; ++i) sum += Acoefs[i]*xcoefs[Acols[i]];
              ycoefs[row] = sum;
            }
        });
    });
            ...
            VectorType& y)
{
  ...
  Queue.submit([&](cl::sycl::handler &cgh) {
      ...
      cgh.parallel_for<class matvec>(ndRange, [=](cl::sycl::nd_item<1> item) {
          ...
          for (GlobalOrdinalType row =  starting_row; row < std::min(n, starting_row + rows_per_item); row++)
            {
              GlobalOrdinalType row_start = row*row_len;
              GlobalOrdinalType row_end   = row_start+row_len;
              ScalarType sum = beta*ycoefs[row];
              for(LocalOrdinalType i=row_start; i<row_end; ++i) sum += Acoefs[i]*xcoefs[Acols[i]];
              ycoefs[row] = sum;
            }
...
template<typename MatrixType,
         typename VectorType>
void matvec(cl::sycl::queue Queue, MatrixType& A, VectorType& x, VectorType& y)
{
                    ...
                    VectorType& y)
{
  ...
  int n = A.rows.size();
  const LocalOrdinalType* Arowoffsets = A.row_offsets.data();
  const GlobalOrdinalType* Acols      = A.packed_cols.data();
  const ScalarType* Acoefs            = A.packed_coefs.data();
  const ScalarType* xcoefs = x.coefs.data();
        ScalarType* ycoefs = y.coefs.data();
  ScalarType beta = 0;
  ...
  Arowoffsets = A.row_offsets_external.data();
  beta = 1;
